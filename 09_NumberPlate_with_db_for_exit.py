...
path = r'data\08_Car_Exit.mp4' # r'data\08_Car_Entry.mp4'
...
while 1:
    ...
    if len(result) != 0 and result[0][2] > 0.5:
        ...
        exit_date = str(datetime.datetime.now())

        # update data
        cursor.execute(f'''UPDATE NumberPlates set Exit_time="{exit_date}", state="Exit", bill=10.0 where NumberPlate="{text.upper()}"; ''')
        connection.commit()
        ...
        cv2.imshow('LiveStrimeScreen2', frame)
    else:
        # show the video strime
        # cv2.imshow('LiveStrimeScreen', frame)
        pass
